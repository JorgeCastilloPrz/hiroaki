package com.jorgecastillo.hiroaki.data.networkdto
...
fun JacksonArticleDto.toArticle() = Article(title, description, url, urlToImage, publishedAt, source.toSource())

fun Article.toJacksonDto() = JacksonArticleDto(title, description, url, urlToImage, publishedAt, source.toJacksonDto())

fun JacksonSourceDto.toSource() = Source(id, name)

fun Source.toJacksonDto() = JacksonSourceDto(id, name)
