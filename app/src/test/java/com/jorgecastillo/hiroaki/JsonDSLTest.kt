...
class JsonDSLTest : MockServerSuite() {
    ...
    fun respondsJsonDSLNonNested() {
        ...
        val parsedData = service.getSomeNonNestedData().execute().body()
    ...
    fun respondsDSLArrayWithPlainNumeric() {
        ...
        val parsedData = service.getSomeNonNestedDataNumericArray().execute().body()
    ...
    private fun expectedSingleNew(requestPathLengthAsSourceId: Int? = null): List<MoshiArticleDto> = listOf(
            MoshiArticleDto("How to Get Android P's Screenshot Editing Tool on Any Android Phone",
                    ...
                    MoshiSourceDto(if (requestPathLengthAsSourceId != null) "$requestPathLengthAsSourceId" else null, "Lifehacker.com")
            ))
}
